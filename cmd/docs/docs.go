...
import (
	...
	"github.com/khostya/effective-mobile/internal/http/openapi"
...
func init() {
	...
	openapi.AddServer(
		&openapi3.Server{
			URL: fmt.Sprintf("http://%v:%v", apiHost, cfg.HTTP.Port),
		},
	)
	...
	m["basePath"] = "/"
	m["host"] = fmt.Sprintf("http://localhost:%v", cfg.HTTP.Port)
