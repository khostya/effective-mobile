...
func (s *server) json(w http.ResponseWriter, r *http.Request, status int, resp interface{}) {
	...
	slog.Debug("json response", sl.URL(r.URL.RawQuery), sl.Code(uint64(status)))
}

func (s *server) error(w http.ResponseWriter, r *http.Request, status int, err error) {
	switch {
	case status == http.StatusBadRequest:
		slog.Debug("bad request", sl.Err(err), sl.URL(r.URL.RawQuery), sl.Code(http.StatusBadRequest))
		w.WriteHeader(status)
	case errors.Is(err, repoerr.ErrNotFound):
		slog.Debug("not found", sl.Err(err), sl.URL(r.URL.RawQuery), sl.Code(http.StatusNotFound))
...
func (s *server) internalServerError(w http.ResponseWriter, r *http.Request, err error) {
	slog.Error("internal server error", sl.Err(err), sl.URL(r.URL.RawQuery), sl.Code(http.StatusInternalServerError))
